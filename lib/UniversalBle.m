...
- (void)didReceiveObject:(NSDictionary *)object
{
    ...
    if ([self.delegate respondsToSelector:@selector(UniversalBle:didReceiveObject:)]) {
        [self.delegate UniversalBle:self didReceiveObject:object];
    }
}
...
- (void)didConnect
{
    if ([self.delegate respondsToSelector:@selector(UniversalBleDidConnect:)]) {
        [self.delegate UniversalBleDidConnect:self];
    }
}

- (void)didDisconnect
{
    if ([self.delegate respondsToSelector:@selector(UniversalBleDidDisconnect:)]) {
        [self.delegate UniversalBleDidDisconnect:self];
    }
...
- (void)peripheralDidUpdateRSSI:(CBPeripheral *)peripheral error:(nullable NSError *)error
{
    ...
    if (peripheral.RSSI) {
        ...
        if ([self.delegate respondsToSelector:@selector(UniversalBle:didUpdateRSSI:)]) {
            [self.delegate UniversalBle:self didUpdateRSSI:@(_filteredRSSI)];
        }
    }
}
